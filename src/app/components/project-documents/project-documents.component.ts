...
export class ProjectDocumentsComponent implements OnInit {
  ...
  ngOnInit() {
    if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
      ...
      this.userId = userid;
      this.loadDocs();
      this.adminwork.GetDocId(this.token).subscribe({
        next: (data) => {
          if (data.Status) {
  ...
  loadDocs() {
    this.adminJob
      .GetJobDocs(this.token, -1, this.job.JobID, this.selectedType)
      .subscribe((res) => {
        if (res.Status) {
  ...
  onSubmitNewDoc() {
    ...
    selectedRoles.forEach((role) => {
      ...
      this.adminJob
        .CreateJobDoc(this.token, jobDetail)
        .subscribe((response) => {
          if (response.Status) {
    ...
    this.uploadFiles( this.selectedFile ? timestamp + '-' + this.selectedFile.name : '' );
